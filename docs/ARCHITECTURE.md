...
**Pool gate (`TREASURE_POOL_MIN` = 30, owner's choice):** treasures only generate — initial fill, or regenerating after a buy empties one or a discard — while the pool (IN_POOL receptacles) holds at least `TREASURE_POOL_MIN`. Below that, slots stay/go empty rather than draw against a thin pool. Checked once per refill pass across all three slots (not once per slot), so three slots each wanting up to `TREASURE_SIZE_MAX` can't deplete a pool sitting right at the threshold and leave some slots filled and others mysteriously empty within the same refill. `TreasureService(pool_min=...)` is constructor-overridable: trial sandboxes (`GameContext.treasure_service()`, `is_trial=True`) pass `pool_min=0`, since they seed a small curated demo set (`services/trial.py:STARTER_REWARDS`, 6 rewards) and are meant to show the full loop immediately rather than simulate a real backlog.
